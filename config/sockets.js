...
module.exports = function(server) {
  ...
  io.on('connection', function (socket) {
    ...
    /* Receives scene events from client-based sockets */
    socket.on('scene:recv', function (sceneData) {
        if(verbose) console.log('scene', {data: sceneData, sock: socket.id, channel: socks[socket.id] });
        socket.broadcast.to(socks[socket.id]).emit('scene:send', { sceneData: sceneData });
    });
