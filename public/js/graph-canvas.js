d3.graph_canvas = function(canvas, W, H, data) {
	...
	// nodes by index. If target and source are strings, then they are referring
	// to IDs of the nodes and not index in the array

	// convert link source and target to integers
	...
	if (data.coord_system_type == "window") {
		...
		if (data.window) {
			...
			xExtent = d3.extent(nodes.filter(function(d) {
				return d.location;
			}), function(d, i) {
				return d.location[0];
			});
			yExtent = d3.extent(nodes.filter(function(d) {
				return d.location;
			}), function(d, i) {
				return d.location[1];
			});
	...
	function ticked() {
		if (fixedNodeCount == nodes.length)
			simulation.alpha(0);
		context.save();
		context.clearRect(0, 0, canvas.attr("width"), canvas.attr("height"));
		context.translate(transform.x, transform.y); //<-- this always applies a transform
...
y:
	...
	function drawSelfLink(d) {
		...
		var ctrl1 = {x:
				d.source.x,
				y:
				d.source.y - 8 * BridgesVisualizer.selfEdge(d.source.size)
			};
		var ctrl2 = {x:
				d.source.x + 8 * BridgesVisualizer.selfEdge(d.source.size),
				y:
				d.source.y
			};
