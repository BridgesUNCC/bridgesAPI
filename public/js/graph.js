...
//
//Based loosely on bostock's example and http://bl.ocks.org/d3noob/5141278

// This function is responsible for rendering all node-link based structures, such
// as all types of linked lists, linked list based stacks and queues  and  graphs

// Last update: Aug 5, 2025 (KRS)
//
d3.graph = function(svg, W, H, data) {
	...
	// otherwise, find the extent of the nodes for the transformation
	if (data.coord_system_type == "window") {
		let xExtent, yExtent, viewportX, viewportY;
		...
		if (data.window) {
			...
			xExtent = d3.extent(data.nodes.filter(function(d) {
				return d.location;
			}), function(d, i) {
				return d.location[0];
			});
			yExtent = d3.extent(data.nodes.filter(function(d) {
				return d.location;
			}), function(d, i) {
				return d.location[1];
			});
	...
	let selflinks = data.links.filter(function(d) {
		return d.target == d.source;
	});
	...
	if (selflinks.length > 0) {
		let selfLinkG = svgGroup.selectAll(".selflink")
	...
	node.append('path')
		...
		.attr("d", 
			d3.symbol().type(function(d) {
				return BridgesVisualizer.shapeLookup(d.shape);
			})
			.size(function(d) {
				return BridgesVisualizer.scaleSize(d.size) || 1;
			}))
	...
	function ticked() {
		...
		if (selflinks.length > 0) {
			...
			selfLinkG.selectAll("text")
				.attr("transform", function(d, i) {
					let myNode = nodes[d.source];
					return "translate(" + (myNode.x + 10) + "," + (myNode.y - 10) + ")";
				});
		}

		linkG.selectAll("path").attr("d", function(d) {
			let x1 = d.source.x,
				y1 = d.source.y,
				x2 = d.target.x,
				y2 = d.target.y,
		...
		linkG.selectAll("text").attr("transform", function(d, i) {
			let ctrl = getControlPoint(d.target, d.source);
