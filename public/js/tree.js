	...
	function draw(source) {
		...
		*/
		...
		.on("mouseover", function(evt, d) {
			BridgesVisualizer.textMouseover(d.data.name);
		} )
		.on("mouseout", function(evt, d) {
			BridgesVisualizer.textMouseout(d)
		});
		...
		.attr("d", d3.symbol()
		.type(function(d) {
			return BridgesVisualizer.shapeLookup(d.data.shape);
		})
		.size(function(d) {
			return BridgesVisualizer.scaleSize(d.data.size) || 1;
		})
		)
		...
		.text( function( d ) {
			if ( (d && d.data.name && d.data.name != "NULL") ) {
				return d.data.name;
			}
			else
				return "";
		...
		*/
		...
		.style("stroke", function(d, i) {
			if (d.data.linkProperties)
				return BridgesVisualizer.getColor(d.data.linkProperties.color);
			return "#ccc";
		})
		.style("stroke-width", function(d, i) {
			if (d.data.linkProperties)
				return BridgesVisualizer.strokeWidthRange(d.data.linkProperties.thickness);
