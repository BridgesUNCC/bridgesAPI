...
function movementTick(camera){
  ...
  if(camera.cameraType == 'fps'){
    if (keyState[38] || keyState[87]){
      forwardX += xDirection; // move in forward in direction camera
      eye[1] += 0.01 * Math.cos(bobs);
      forwardZ += zDirection; // move in forward in direction camera
      bobs += 0.2;//for bobbing during walking
    }
    if (keyState[40] || keyState[83]){
      forwardX -= xDirection; // move in backwards in direction camera
      eye[1] += 0.01 * Math.cos(bobs);
      forwardZ -= zDirection; // move in backwards in direction camera
      bobs -= 0.2;//for bobbing during walking
    }
    ...
    if (keyState[32] || keyState[32]){
      //move up
      eye[1] += 1.0
    }
    if (keyState[17] || keyState[17]){
      //move down
      eye[1] -= 1.0
    }
